...
function custom_form_Fields($id, $edit_id = null, $col_sm = null)
{
	...
	$all_users_field = $CI->db->where('designations_id', 1)->get('tbl_account_details')->result();
    ...
    if (!empty($all_field)) {
        foreach ($all_field as $v_fileds) {
            if (!empty($v_fileds->visible_for_admin)) {
                ...
                if ($v_fileds->field_type == 'dropdown' && $v_fileds->status == 'active' && $v_fileds->field_label != 'Head' ) {
				...
				else if($v_fileds->field_type == 'dropdown' && $v_fileds->status == 'active' && $v_fileds->field_label == 'Head' )
				{
					$idList="";
					for($i=0 ; $i < sizeof($all_users_field) ; $i++)
					{
						$optionValue=$all_users_field[$i]->user_id;
						$displayValue=$all_users_field[$i]->user_id . " | " . $all_users_field[$i]->fullname ." | ". $all_users_field[$i]->province;
						$idList.= '<option value="' . $optionValue . '" ' . (!empty($editValue) && $editValue == $optionValue ? "selected" : null) . '>' . $displayValue . '</option>';
					}
